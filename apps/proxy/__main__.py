...
BUF_SIZE = 2048
TIMEOUT_SOCKET = 5
LOCAL_ADDR = os.environ.get("SOCKS_ADDR", "0.0.0.0")
LOCAL_PORT = os.environ.get("SOCKS_PORT", "9999")
...
def request_client(wrapper):
    ...
    # +----+-----+-------+------+----------+----------+
    try:
        s5_request = wrapper.recv(BUF_SIZE)
    ...
    # DOMAIN NAME
        ...
        return False
    print(f"[{time.ctime()}]: {dst_addr.decode()}, {dst_port}")
...
def subnegotiation_client(wrapper):
    ...
    # +----+----------+----------+
    try:
        identification_packet = wrapper.recv(BUF_SIZE)
    except socket.error:
        error()
...
def bind_port(sock):
    ...
    """
    try:
        print('Socks5 Proxy started at {}'.format(str(LOCAL_PORT)))
